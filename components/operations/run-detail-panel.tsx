  ...
  return (
    <div className="stack">
      <div className="grid cols-3">
        ...
        <div className="card stat stat-card">
          <strong>
            {detail.diagnostics.attempts}/{detail.diagnostics.maxAttempts}
          </strong>
          <span>Attempts</span>
        </div>
        <div className="card stat stat-card">
      ...
      <section className="card stack">
        ...
        <div className="list">
          <div className="list-item">
            ...
            <strong>{detail.run.triggerSource}</strong>
          </div>
          <div className="list-item">
            <span>Workflow</span>
            <strong>{detail.run.workflowId}</strong>
          </div>
          <div className="list-item">
            <span>Started</span>
            <strong>{detail.run.startedAt ? new Date(detail.run.startedAt).toLocaleString() : "-"}</strong>
          </div>
          <div className="list-item">
            <span>Finished</span>
            <strong>{detail.run.finishedAt ? new Date(detail.run.finishedAt).toLocaleString() : "-"}</strong>
          </div>
          <div className="list-item">
            <span>Target destination</span>
            <strong>{detail.diagnostics.targetPageId || "-"}</strong>
          </div>
          <div className="list-item">
      ...
      <section className="card stack">
        ...
        {detail.contentItem ? (
          <div className="stack">
            <div className="list-item">
              <span>Title</span>
              <strong>{detail.contentItem.title}</strong>
            </div>
            <div className="list-item">
              <span>Status</span>
              <strong>{detail.contentItem.status}</strong>
            </div>
            <div className="list-item">
              <span>Destinations</span>
              <strong>{detail.contentItem.destinationIds?.join(", ") || "-"}</strong>
            </div>
            <div className="card" style={{ whiteSpace: "pre-wrap" }}>
              {detail.contentItem.bodyText || "No body text."}
            </div>
          </div>
        ) : (
          <div className="list-item">
            <span className="muted">No linked content item found for this run.</span>
          </div>
        )}
      </section>
      ...
      <section className="card stack">
        ...
        <div className="list">
          {detail.timeline.length ? (
            ...
            <div className="list-item">
              <span className="muted">No timeline events captured for this run yet.</span>
            </div>
          )}
        </div>
      </section>
      ...
      <div className="grid cols-2">
        <section className="card stack">
          ...
          <pre className="card" style={{ overflowX: "auto", whiteSpace: "pre-wrap" }}>
            {prettyJson(detail.run.inputJson)}
          </pre>
        </section>
        <section className="card stack">
          <div className="section-head">
