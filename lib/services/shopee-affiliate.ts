...
function rewriteRawShopeeSpecToBenefit(value: string, product?: ShopeeProductRecord) {
  ...
  if (/ปรับได้\s*\d+\s*(?:โทน|สี|ระดับ|mode|โหมด)/iu.test(cleaned)) {
    if (/โคม|lamp|ไฟ|แสง/.test(haystack)) return "เลือกโทนแสงให้เหมาะกับการอ่านหนังสือหรือทำงานได้ง่าย";
    return "ปรับรูปแบบการใช้งานให้เข้ากับสถานการณ์ได้ง่าย";
  }
  ...
  if (/nylon|ไนลอน/i.test(cleaned)) {
    return /กระเป๋า|bag|เป้|คาดอก/.test(haystack)
      ? "เนื้อผ้าทนต่อการพกของออกนอกบ้านและดูแลง่าย"
      : "วัสดุเหมาะกับการใช้งานบ่อยและดูแลง่าย";
  }
  if (/สแตนเลส|stainless/i.test(cleaned)) {
    return /แก้ว|ขวด|tumbler/.test(haystack)
      ? "วัสดุช่วยให้พกเครื่องดื่มได้มั่นใจและทำความสะอาดง่าย"
      : "วัสดุแข็งแรง เหมาะกับการใช้งานบ่อย";
  }
