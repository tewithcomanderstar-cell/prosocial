...
export async function createAutoPostRecords(input: StartAutoPostRecordInput) {
  ...
  await AutoPostConfig.findByIdAndUpdate(input.configId, {
...
export async function updateAutoPostRecords(input: UpdateAutoPostRecordInput) {
  const config = await AutoPostConfig.findById(input.configId).lean<ConfigRefs | null>();
  if (!config) {
    return null;
  }
