...
function readShopeeProductsRouteSource() {
  return readFileSync(resolve(dirname(fileURLToPath(import.meta.url)), "../../app/api/shopee/products/route.ts"), "utf8");
}
...
function testShopeeCategoryNormalization() {
  ...
  assert.ok(getAllShopeeSpecificCategoryValues().length > 10);
  assert.equal((getAllShopeeSpecificCategoryValues() as string[]).includes("all"), false);
  assert.ok(expandShopeeDiscoveryCategories(["all"]).includes("home_living"));
  assert.ok(expandShopeeDiscoveryCategories(["all"]).includes("beauty"));
  assert.deepEqual(expandShopeeDiscoveryCategories(["Automotive", "Sports & Outdoors"]), ["automotive", "sports"]);
...
function testStoryboardCaptionSourceUsesProductEntityGuards() {
  ...
  assert.ok(captionSource.includes("buildShopeeStoryboardHook"));
  assert.ok(captionSource.includes("affiliateLink.trim()"));
  assert.equal(captionSource.includes("buildShopeeStoryboardCtaLine"), false);
...
function testShopeeFilterOnlySelectionGuards() {
  ...
  assert.ok(source.includes("SHOPEE_ALL_PRODUCTS_CATEGORY_FETCH_ATTEMPT"));
  assert.ok(source.includes("category_keyword_random_page"));
  assert.ok(source.includes("category_broad_first_page"));
  assert.equal(selectionSource.includes("getRecentlyPostedProductKeys"), false);
  assert.equal(selectionSource.includes("getActiveReservedProductKeys"), false);
  assert.equal(selectionSource.includes("wasProductRecentlyPosted"), false);
  assert.equal(selectionSource.includes("reserveShopeeProductKey"), false);
  assert.equal(selectionSource.includes("duplicate_product_48h"), false);
  assert.equal(selectionSource.includes("reserved_product"), false);
  assert.equal(selectionSource.includes("english_only_product_name"), false);
  assert.equal(selectionSource.includes("out_of_stock"), false);
...
function testShopeeCaptionFallbackGenerationGuards() {
  ...
  assert.ok(aiSource.includes("viral"));
  assert.ok(aiSource.includes("Hook 1"));
  ...
  assert.equal(captionSource.includes("thai_social_caption_missing_review_bullets"), false);
  assert.ok(captionSource.includes("thai_social_caption_missing_emoji"));
...
function testShopeeProductIntelligenceLayerGuards() {
  ...
  assert.ok(source.includes("source: \"text\" | \"vision_rescue\" | \"merged\" | \"product_intelligence\""));
  assert.ok(source.includes("const productIntelligence = getShopeeProductIntelligenceFromProduct(product)"));
  assert.ok(source.includes("source: \"product_intelligence\""));
  assert.ok(source.includes("toilet_deodorizer"));
  assert.ok(source.includes("bio_deodorizer_powder"));
  assert.ok(source.includes("ดับกลิ่นส้วม"));
...
function testShopeeCaptionQualityAndSoldThresholdGuards() {
  ...
  for (const sourceTag of ["sold_50_plus", "sold_100_plus", "sold_500_plus", "sold_1000_plus", "sold_1500_plus", "sold_2000_plus"]) {
  ...
  for (const threshold of ["50", "100", "500", "1000", "1500", "2000"]) {
  ...
  assert.equal(sourceBetween(source, "function getShopeeProductFilterRejectionReason", "const PRODUCT_SELECTION_REJECTION_REASONS").includes("duplicate_product_48h"), false);
  assert.ok(panelSource.includes("SOLD 50+"));
  assert.ok(panelSource.includes("SOLD 100+"));
...
function testShopeeAllProductsAndCooldownGuards() {
  ...
  assert.ok(source.includes("page: randomPage"));
  assert.ok(source.includes("fetch_all_selected_categories_with_new_random_page"));
  assert.ok(source.includes("dedupeShopeeProducts"));
  assert.ok(source.includes("getShopeeCanonicalProductKey"));
  assert.ok(source.includes("return getShopeeCanonicalProductKey(product) || getShopeeProductIdentity(product)"));
  assert.equal(sourceBetween(source, "export async function selectShopeeProductsForPages", "export function buildShopeeImagePrompt").includes("getRecentlyPostedProductKeys"), false);
  assert.equal(sourceBetween(source, "export async function selectShopeeProductsForPages", "export function buildShopeeImagePrompt").includes("getActiveReservedProductKeys"), false);
  assert.equal(sourceBetween(source, "export async function selectShopeeProductsForPages", "export function buildShopeeImagePrompt").includes("reserveShopeeProductKey"), false);
...
function testShopeeProductFilteringAuditGuards() {
  const source = readShopeeAffiliateServiceSource();
  ...
  assert.ok(source.includes("PRODUCT_SELECTION_FUNNEL"));
  for (const field of [
    "afterNormalize",
    "afterImageFilter",
    "afterUrlFilter",
    ...
    assert.ok(funnelSource.includes(field), `Missing PRODUCT_SELECTION_FUNNEL field ${field}`);
  }
  ...
  assert.equal(filterSource.includes("missing_product_url"), false);
  assert.equal(filterSource.includes("out_of_stock"), false);
  assert.equal(filterSource.includes("blocked_category"), false);
  assert.equal(filterSource.includes("duplicate_product_48h"), false);
  assert.equal(filterSource.includes("reserved_product"), false);
  assert.ok(filterSource.indexOf("missing_image") < filterSource.indexOf("below_min_price"));
  ...
  assert.ok(selectionSource.includes("expandShopeeDiscoveryCategories"));
...
function testShopeeMultiCategorySelectionRescueGuards() {
  ...
  assert.ok(selectionSource.includes("discoveryCategories.length > 1"));
...
testShopeeProductFilteringAuditGuards();
