...
function getRetryWithNextProductReason(error: unknown) {
  if (isShopeeShortLinkFailure(error)) {
    return {
      reason: "short_link_invalid",
      step: "PRODUCT_SKIPPED_SHORT_LINK_INVALID",
      message: "Skipping product because Shopee short link is invalid"
    };
  }
  ...
  if (
    ...
    message.includes("caption generation failed") ||
    message.includes("caption validation failed") ||
    message.includes("generated caption is invalid") ||
    message.includes("invalid generated caption") ||
    message.includes("ai content validation failed") ||
    message.includes("ai content rejected") ||
    message.includes("product context too weak") ||
    message.includes("missing product description")
  ) {
    return {
      reason: "caption_generation_failed",
      step: "PRODUCT_SKIPPED_CAPTION_FAILED",
      message: "Skipping product because caption generation or validation failed"
    };
  }

  return {
    reason: "safety_rejected_or_image_failed",
    step: "PRODUCT_SKIPPED_IMAGE_FAILED",
    message: "Skipping product because image generation or validation failed"
  };
}
...
function getShopeeAttemptFailureDiagnostics(error: unknown) {
  const code = getAutoPostErrorCode(error);
  const message = getAutoPostErrorMessage(error);
  const responseSummary = getAutoPostResponseSummary(error);
  const diagnostics = {
    productUnderstandingError: null as string | null,
    captionGenerationError: null as string | null,
    affiliateLinkError: null as string | null,
    templatePostCreationError: null as string | null,
  ...
  if (isShopeeShortLinkFailure(error) || normalized.includes("affiliate") || normalized.includes("short link")) {
    diagnostics.affiliateLinkError = message || responseSummary || code;
  }
  if (normalized.includes("template_post") || normalized.includes("template post")) {
    diagnostics.templatePostCreationError = message || responseSummary || code;
  }
  if (normalized.includes("mongo") || normalized.includes("database") || normalized.includes("save")) {
    diagnostics.databaseSaveError = message || responseSummary || code;
  }
  return diagnostics;
}
...
export function classifyAutoPostError(error: unknown): AutoPostErrorClassification {
  ...
  if (
    ...
    message.includes("product context too weak") ||
    message.includes("missing product description") ||
    message.includes("invalid shopee short link")
  ) {
    return "retry_with_next_product";
  }
...
async function prepareSingleShopeePackageWithProductAttempts(input: {
  ...
  for (let attempt = 1; attempt <= AUTO_POST_MAX_PRODUCT_ATTEMPTS; attempt += 1) {
    ...
    for (let sameProductAttempt = 1; sameProductAttempt <= AUTO_POST_SAME_PRODUCT_RETRIES; sameProductAttempt += 1) {
      try {
        ...
        const failureDiagnostics = getShopeeAttemptFailureDiagnostics(error);
        ...
        if (classification === "retry_with_next_product") {
          ...
          const shortLinkFailure = skipInfo.reason === "short_link_invalid";
          ...
          await logShopeeStep({
            ...
            metadata: {
              ...
              currentProductName: selected.product.productName,
              productAttempt: attempt,
              ...failureDiagnostics,
              failedRules: captionFailedRules.map((item) => item.rule),
              failedRuleMessages: captionFailedRules.map((item) => item.message),
              skippedProductsCount: skippedProducts.length,
          ...
          await logShopeeStep({
            ...
            step: skipInfo.step,
            status: "skipped",
            message: shortLinkFailure ? `Skipping product ${productId}` : skipInfo.message,
            pageId: selected.pageId,
            productId,
            error,
            metadata: {
          ...
          await logShopeeStep({
            config: input.config,
            step: "SKIPPED_PRODUCT_WITH_REASON",
            status: "skipped",
            message: `Skipped Shopee product ${productId}: ${skipReason}`,
            pageId: selected.pageId,
            productId,
            error,
            metadata: {
  ...
  const finalMessage = `${message}. Rejection summary: ${Object.entries(rejectReasons).map(([reason, count]) => `${reason}=${count}`).join(", ") || "none"}`;
  await logShopeeStep({
    config: input.config,
  ...
  await logShopeeStep({
    ...
    metadata: {
      ...
      skippedProducts,
      lastSkippedReason: skippedProducts.at(-1)?.reason ?? "",
      maxAttempts: AUTO_POST_MAX_PRODUCT_ATTEMPTS,
      maxSearchCycles: AUTO_POST_MAX_SEARCH_CYCLES,
      workflowRunId: input.records.workflowRunId
    }
