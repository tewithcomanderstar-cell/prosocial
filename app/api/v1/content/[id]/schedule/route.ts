...
import { contentIdParamsSchema, scheduleContentSchema } from '@/src/modules/content/content.schemas';
...
export const POST = withRouteHandler(async (request: NextRequest, contextData: { params: Promise<{ id: string }> }) => {
  ...
  const body = parseOrThrow(scheduleContentSchema, await request.json());
  return apiOk(await service.scheduleContent(context, params.id, body));
});
